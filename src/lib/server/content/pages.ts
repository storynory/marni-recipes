// src/lib/server/content/Page.ts
import fs from 'fs';
...
export function getAllPagePosts(): PagePost[] {
    // Matches your Sveltia folder: "Pages"
    const dir = collectionPath('pages');

    if (!fs.existsSync(dir)) return [];

    const files = listMarkdownFiles(dir);

    const posts: PagePost[] = files.map((file) => {
